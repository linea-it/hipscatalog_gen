...
def filter_remainder_by_coverage_partition(
    ...
    if order_desc:
        mask = np.array([(t is None) or (s < t) for s, t in zip(sc.values, thr, strict=False)], dtype=bool)
    else:
        mask = np.array([(t is None) or (s > t) for s, t in zip(sc.values, thr, strict=False)], dtype=bool)
...
def _candidates_by_coverage_partition(
    ...
    if isinstance(k_per_cov, dict):
        k_map = pd.Series(k_per_cov, dtype="float64")
        need_vec = pdf["__icov__"].map(k_map).fillna(0.0).astype("int64") + int(tie_buffer)
        need_vec = need_vec.clip(lower=0)
        out = pdf[pdf["__rk__"] < need_vec.to_numpy()]
    else:
...
def _reduce_coverage_exact_dask(
    ...
    # Prepare typed holders with explicit casts so mypy não reclama.
    if is_dict:
        k_map: Optional[Dict[int, float]] = cast(Dict[int, float], k_per_cov)
        k_scalar: Optional[int] = None
    else:
        k_map = None
        k_scalar = int(cast(int, k_per_cov))

    # Scalar and non-positive → empty result of same schema.
    if (not is_dict) and (k_scalar is not None) and k_scalar <= 0:
    ...
    def _take_topk(group: pd.DataFrame) -> pd.DataFrame:
        ...
        if is_dict:
            assert k_map is not None  # for type checkers
            icov_val = int(group["__icov__"].iloc[0])
            k = int(k_map.get(icov_val, 0))
        else:
            assert k_scalar is not None  # for type checkers
            k = int(k_scalar)
    ...
    if hasattr(ddf_like, "groupby"):
        cols_all = list(ddf_like.columns)
        return ddf_like.groupby("__icov__", group_keys=False)[cols_all].apply(_take_topk, meta=meta)
...
def apply_fractional_k_per_cov(
    ...
    rng = np.random.default_rng()
    kept_parts_rand: List[pd.DataFrame] = []
    dropped_parts_rand: List[pd.DataFrame] = []
    ...
    for icov, g in selected.groupby("__icov__"):
        ...
        k_local_desired = _k_desired_for_icov(int(icov), n_avail)
        ...
        if len(final_keep) > 0:
            kept_parts_rand.append(final_keep)
        if len(final_drop) > 0:
            dropped_parts_rand.append(final_drop)

    kept_df = pd.concat(kept_parts_rand, ignore_index=False) if kept_parts_rand else selected.iloc[0:0]
    dropped_df = (
        pd.concat(dropped_parts_rand, ignore_index=False) if dropped_parts_rand else selected.iloc[0:0]
    )
    return kept_df, dropped_df
