...
def test_prepare_keep_invalid_values_maps_and_expands_range(diag_ctx, log_capture):
    """Tests that keep_invalid_values maps NaN/Inf to sentinel and keeps them."""
    _, log_fn = log_capture
    ...
    assert cfg.algorithm.mag_max >= 121.0
    assert sorted(result["__mag__"].unique().tolist()) == [20.0, 120.0, 121.0]
